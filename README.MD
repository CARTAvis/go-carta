...
The spawner will attempt to launch the worker executable using `sudo` and must be given the ability to password-lessly launch instances of the executable for permitted users.  This is likely to require [modifying the sudoers configuration](https://www.sudo.ws/man/1.9.0/sudoers.man.html).  Below is an example of a sudoers rule that will allow the user `carta` to launch instances of the backend for any user in the `carta-users` group as an illustration of the sort of permissions required.
